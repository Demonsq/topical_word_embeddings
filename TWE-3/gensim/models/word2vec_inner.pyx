...
ctypedef void (*fast_sentence_sg_hs_ptr) (
    ...
    REAL_t *syn0, REAL_t *syn0_topic, REAL_t *syn1, const int size, const int size2,
    const np.uint32_t word2_index, const np.uint32_t topic2_index,  const REAL_t alpha, REAL_t *work) nogil
...
cdef void fast_sentence2_sg_hs(
    ...
    cdef long long a, b
    cdef long long row1 = word2_index * size, row2
    cdef long long row_topic = topic2_index * size
    cdef REAL_t f, g

    for a in range(size2):
        work[a] = <REAL_t>0.0
    for b in range(codelen):
        row2 = word_point[b] * size2
        f = <REAL_t>0.0
        for a in range(size):
            f += syn0[row1 + a] * syn1[row2 + a]
        for a in range(size, size2):
            f += syn0_topic[row_topic + a - size] * syn1[row2 + a]
        ...
        for a in range(size, size2):
            syn1[row2 + a] += g * syn0_topic[row_topic + a - size]
    for a in range(size):
        syn0[row1 + a] += work[a]
    for a in range(size, size2):
        syn0_topic[row_topic + a - size] += work[a]
...
def train_sentence_sg(model, sentence, alpha, _work):
    ...
    cdef int size1 = model.layer1_size
    cdef int size2 = model.layer2_size
    cdef int codelens[MAX_SENTENCE_LEN]
    cdef np.uint32_t topic_index[MAX_SENTENCE_LEN]
    ...
    for i in range(sentence_len):
        ...
        if word is None:
            ...
            word_index[i] = sentence[i][1]
            topic_index[i] = sentence[i][2]
        ...
        for i in range(sentence_len):
            ...
            for j in range(j, k):
                if j == i or codelens[j] == 0:
                    continue
                if hs:
                    fast_sentence_sg_hs(points[i], codes[i], codelens[i], syn0, syn0_topic,syn1,size1, size2, word_index[j], topic_index[j], _alpha, work)
            #if negative:
